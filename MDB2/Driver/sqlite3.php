...
class MDB2_Driver_sqlite3 extends MDB2_Driver_Common
{
    ...
    function __construct()
    {
        ...
        $this->options['database_extension'] = '.sqlite3';
    ...
    public function escape($text, $escape_wildcards = false)
    {
		if($this->connection){
			...
			$text = str_replace("'","''",$text);//TODO; more
		}
    ...
    function getServerVersion($native = false)
    {
        ...
        if ($this->connected_server_info) {
            ...
            $server_info = @SQLite3::version();
            $server_info = $server_info['versionString'];
        }
        if (!$server_info) {
            return $this->raiseError(MDB2_ERROR_UNSUPPORTED, null, null,
                'Requires either the "server_version" option or the SQLite3 class', __FUNCTION__);
        }
        // cache server_info
...
class MDB2_Statement_sqlite3 extends MDB2_Statement_Common
{
    ...
    {
		if($this->db->getOption('emulate_prepared')){
			$this->values = $values;
			return MDB2_OK;
		}
